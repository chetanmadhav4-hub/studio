...
import { Megaphone, Save, Loader2, AlertCircle, Zap } from 'lucide-react';
  ...
  return (
    <div className="h-full overflow-y-auto overscroll-contain touch-pan-y custom-scrollbar pb-32">
      <div className="max-w-2xl mx-auto space-y-6 pt-4">
        ...
        <Card className="border-none shadow-2xl overflow-hidden rounded-[2.5rem] bg-white mx-4 sm:mx-0">
          ...
          <CardContent className="p-8 space-y-8">
            ...
            <div className="p-6 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200 space-y-3">
              ...
              {isActive ? (
                <div className="bg-primary/5 border border-primary/10 p-5 rounded-2xl flex items-start gap-4 shadow-sm">
          ...
          <CardFooter className="bg-slate-50 border-t p-6 flex flex-col sm:flex-row justify-between items-center gap-4">
             <Link href="/" className="w-full sm:w-auto"><Button variant="ghost" className="w-full text-xs font-bold uppercase tracking-widest">Cancel</Button></Link>
            <Button 
              onClick={handleSave} 
              disabled={isSaving || isLoading}
              className="w-full sm:w-auto gap-2 px-10 h-12 font-black uppercase tracking-wider rounded-2xl shadow-xl hover:scale-105 transition-transform"
            >
