      ...
      <div className="max-w-md mx-auto space-y-4">
        <div className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-slate-900">Admin Control</h1>
        ...
        <Tabs defaultValue="pending" className="w-full">
          <TabsList className="grid w-full grid-cols-2 h-12 rounded-xl bg-white p-1 shadow-sm border">
            <TabsTrigger value="pending" className="rounded-lg data-[state=active]:bg-primary data-[state=active]:text-white font-bold text-xs gap-2">
              <Clock className="w-3.5 h-3.5" />
              Pending ({pendingOrders.length})
            </TabsTrigger>
            <TabsTrigger value="completed" className="rounded-lg data-[state=active]:bg-primary data-[state=active]:text-white font-bold text-xs gap-2">
          ...
          <TabsContent value="pending" className="mt-4 space-y-4">
            {isLoading ? (
              <div className="py-12 flex flex-col items-center gap-2">
                <Loader2 className="w-8 h-8 animate-spin text-primary" />
                <p className="text-xs text-slate-500 font-medium">Loading new orders...</p>
              </div>
            ) : pendingOrders.length === 0 ? (
              <div className="bg-white rounded-2xl p-16 text-center space-y-3 border shadow-sm">
                <CheckCircle2 className="w-12 h-12 text-emerald-100 mx-auto" />
                <p className="text-sm text-slate-400 font-medium">Koi pending order nahi hai!</p>
              </div>
            ) : (
          ...
          <TabsContent value="completed" className="mt-4 space-y-4">
            {completedOrders.length === 0 ? (
              <div className="bg-white rounded-2xl p-16 text-center space-y-3 border shadow-sm">
                ...
                <p className="text-sm text-slate-400 font-medium">History khali hai.</p>
              </div>
            ) : (
...
function OrderCard({ order, onCopy, onAction, copiedId, isProcessing, showActions }: any) {
  return (
    <Card className="border-none shadow-md rounded-2xl bg-white overflow-hidden">
      <CardHeader className="border-b p-4 bg-slate-50/50 flex flex-row items-center justify-between space-y-0">
        ...
        <div className="text-right">
          ...
          <Badge variant="secondary" className="font-bold text-[10px] bg-primary/10 text-primary border-none">
      ...
      <CardContent className="p-4 space-y-4">
        ...
        <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 flex items-center justify-between">
          <div>
            ...
            <p className="font-mono text-xs font-bold text-slate-800 tracking-wider">{order.utrId || 'PENDING'}</p>
          </div>
          <Button variant="ghost" size="icon" onClick={() => onCopy(order.utrId || '', order.id)} className="h-8 w-8">
        ...
        <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 space-y-2">
          ...
          <p className="text-[11px] text-blue-600 font-medium break-all line-clamp-1">{order.targetLink}</p>
          <Button asChild variant="outline" className="w-full h-8 text-xs font-bold gap-2 border-blue-200 text-blue-600 hover:bg-blue-50">
        ...
        {showActions ? (
          <div className="grid grid-cols-2 gap-3 pt-2">
            <Button 
              ...
              variant="outline" 
              className="h-10 text-xs font-bold gap-2 text-red-500 border-red-100 hover:bg-red-50 rounded-xl"
            >
            ...
            <Button 
              ...
              disabled={isProcessing}
              className="h-10 text-xs font-bold gap-2 bg-emerald-600 hover:bg-emerald-700 rounded-xl shadow-lg shadow-emerald-200"
            >
          ...
          <div className="pt-2">
            <Badge className={`w-full h-9 flex items-center justify-center rounded-xl font-bold uppercase tracking-wider text-[10px] ${
              order.status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-red-100 text-red-700 border-red-200'
            }`}>
