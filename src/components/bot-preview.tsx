...
export function BotPreview({ isAppMode = false }: BotPreviewProps) {
    ...
    return (
      <div className="flex flex-col gap-1">
        ...
        {hasForm && (
          <div className="mt-3 p-4 bg-slate-50 dark:bg-zinc-900 rounded-[1.5rem] border-2 border-primary/20 shadow-lg space-y-3">
            <Input 
              ...
              className="h-9 text-[11px] font-bold dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-200 rounded-xl" 
              value={formLink} 
            ...
            <Button 
              size="sm" 
              className="w-full bg-[#00A884] hover:bg-[#008F6F] h-10 text-[11px] font-black uppercase tracking-wider rounded-xl shadow-lg" 
        ...
        {optionLines.map((optLine, i) => {
          ...
          return (
            <button key={i} onClick={() => handleSend(optionText)} className="mt-2 w-full py-2.5 px-4 font-black text-[11px] rounded-xl border-2 bg-white dark:bg-zinc-900 text-primary dark:text-accent border-primary/10 hover:border-primary/40 hover:bg-primary/5 transition-all flex items-center justify-center gap-2 shadow-sm active:scale-95 uppercase tracking-tight">
  ...
  return (
    <div className={cn(
      ...
      <div className="bg-[#075E54] dark:bg-zinc-900 text-white py-2.5 px-4 flex items-center gap-3 shrink-0 shadow-md z-20">
        <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center border-2 border-white/10">
          <Bot className="w-5 h-5" />
        </div>
        <div className="text-left flex-1">
          <h2 className="text-xs sm:text-sm font-black text-white leading-none">InstaFlow Bot</h2>
          <div className="flex items-center gap-1 mt-0.5">
            <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse"></span>
            <p className="text-[9px] sm:text-[10px] text-emerald-100 font-bold">Online & Working</p>
          </div>
        </div>
        {!isAppMode && (
          <Link href="/login">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-[url('https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png')] bg-repeat dark:bg-blend-multiply dark:bg-zinc-950 scroll-smooth custom-scrollbar z-10">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={cn(
              "max-w-[85%] rounded-2xl px-4 py-2.5 text-xs shadow-md border dark:border-zinc-700",
              msg.role === "user" 
        ...
        {loading && (
          <div className="flex items-center gap-2 text-[10px] font-black italic opacity-60 dark:text-zinc-200 animate-pulse bg-white/50 dark:bg-zinc-800/50 w-fit px-3 py-1 rounded-full">
            <Bot className="w-3.5 h-3.5" /> Bot is thinking...
          </div>
        )}
      </div>
      ...
      <div className="p-3 bg-[#F0F2F5] dark:bg-zinc-900 flex gap-2 border-t dark:border-zinc-800 shrink-0 shadow-lg z-20">
        <Input 
          ...
          disabled={!user} 
          className="bg-white dark:bg-zinc-800 dark:text-zinc-200 dark:border-zinc-700 rounded-2xl h-10 sm:h-11 px-4 text-xs font-bold" 
        />
        <Button onClick={() => handleSend()} disabled={loading || !user} size="icon" className="rounded-full bg-[#00A884] hover:bg-[#008F6F] h-10 w-10 sm:h-11 sm:w-11 shrink-0 shadow-lg active:scale-90 transition-all">
